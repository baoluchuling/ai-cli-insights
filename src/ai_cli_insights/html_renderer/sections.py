...
def _delta_descriptor(delta: float) -> tuple[str, str]:
    ...
    if delta > 0:
        return f"up {format_metric(abs(delta))}", "delta-up"
    return f"down {format_metric(abs(delta))}", "delta-down"
...
def _series_summary_pill(item: dict, previous_window_item: dict | None) -> str:
    ...
    return (
        ...
        '<span class="trend-pill-subvalue">Latest</span>'
        '</div>'
        f'<div class="trend-summary-pill {prev_step_class}">'
        '<span class="trend-pill-label">Prev Step</span>'
        f'<span class="trend-pill-value">{html.escape(prev_step_text)}</span>'
        '<span class="trend-pill-subvalue">vs previous point</span>'
        '</div>'
        f'<div class="trend-summary-pill {prev_window_class}">'
        '<span class="trend-pill-label">Prev Window</span>'
        f'<span class="trend-pill-value">{html.escape(prev_window_text)}</span>'
        '<span class="trend-pill-subvalue">vs previous range</span>'
        '</div>'
    )
...
def build_comparison_table(data: AnalyzedData, meta: ReportMeta) -> str:
    ...
    if meta.compare_sources:
        rows = [
            ("Session 数量", str(claude.get("sessions", 0)), str(codex.get("sessions", 0))),
            ...
            ("Tool Top5", fmt_tool_list(claude.get("top_tools", [])), fmt_tool_list(codex.get("top_tools", []))),
...
def build_period_section(period_comparison: PeriodComparison | None, meta: ReportMeta) -> str:
    ...
    if meta.compare_sources:
        ...
        return (
            f"{heading_html('h2', 'Period over Period', 'period', 'section-period')}"
            ...
            "<table class=\"comparison-table\"><thead><tr><th>工具</th><th>Sessions</th><th>平均时长</th><th>平均消息数</th></tr></thead><tbody>"
            + "".join(rows)
            + "</tbody></table>"
        )
    ...
    return (
        ...
        f"<tr><td>Sessions</td><td>{delta_text(cur.get('sessions', 0), prev.get('sessions', 0))}</td></tr>"
...
def _mode_copy(variant: str) -> dict[str, str]:
    ...
    if variant == "codex":
        return {
            "lane": "Execution lane",
            "win": "Execution signal",
            "friction": "Delivery risk",
            "recommendation": "Runbook rule",
            "library": "Runbook prompt",
            "feature": "Operator template",
            "pattern": "Execution pattern",
        }
    return {
        "lane": "Workstream",
        "win": "Platform signal",
        "friction": "Coordination risk",
        "recommendation": "Workflow rule",
        "library": "Shared prompt",
        "feature": "Prompt template",
        "pattern": "Workflow pattern",
    }
        ...
        dedent(
            ...
            <div class="project-area">
              <div class="area-header">
                ...
                <span class="area-count">{card['count']} sessions | {html.escape(card['split'])}</span>
              </div>
...
def render_prompt_cards(cards: list[dict], kind: str, variant: str = "all") -> str:
    ...
    starter_label = "Getting started:" if kind == "feature" else "Try this next:"
        ...
        dedent(
            ...
            <div class="{class_name}">
              ...
              <div class="copyable-prompt-section">
                <div class="prompt-label">Paste into Claude or Codex:</div>
    ...
    return "\n".join(
        dedent(
            ...
            <div class="project-area">
              <div class="area-header">
                <div class="area-heading">
                  <span class="area-mode-pill">Execution read</span>
...
def render_friction_details(data: AnalyzedData) -> str:
    ...
    return (
        f"<details class='project-area'><summary class='area-name'>{title_with_help('打开 Claude friction 与 outcome 统计', 'claude_friction_details')}</summary><div style='margin-top:16px'><div class='charts-row'><div class='chart-card'>{block_title_html('Claude Top Friction', 'chart_claude_friction', 'chart-title')}<table class='comparison-table'><thead><tr><th>类型</th><th>次数</th></tr></thead><tbody>{top_friction_html}</tbody></table></div><div class='chart-card'>{block_title_html('Claude Outcome', 'chart_claude_outcome', 'chart-title')}<table class='comparison-table'><thead><tr><th>Outcome</th><th>Sessions</th></tr></thead><tbody>{outcome_html}</tbody></table></div></div><div class='project-areas'>{friction_session_html}</div></div></details>"
    )
...
def render_stats_row(data: AnalyzedData, meta: ReportMeta) -> str:
    ...
    if meta.compare_sources:
        return (
            f"<div class='stat'><div class='stat-value'>{claude.get('sessions', 0)}</div><div class='stat-label'>Claude Sessions</div></div>"
            f"<div class='stat'><div class='stat-value'>{codex.get('sessions', 0)}</div><div class='stat-label'>Codex Sessions</div></div>"
            f"<div class='stat'><div class='stat-value'>{claude.get('avg_user_messages', 0)}</div><div class='stat-label'>Claude Msgs/Session</div></div>"
            f"<div class='stat'><div class='stat-value'>{codex.get('avg_user_messages', 0)}</div><div class='stat-label'>Codex Msgs/Session</div></div>"
            f"<div class='stat'><div class='stat-value'>{claude.get('avg_duration_min', 0)}</div><div class='stat-label'>Claude Avg Min</div></div>"
            f"<div class='stat'><div class='stat-value'>{codex.get('avg_duration_min', 0)}</div><div class='stat-label'>Codex Avg Min</div></div>"
        )
    return (
        f"<div class='stat'><div class='stat-value'>{primary.get('sessions', 0)}</div><div class='stat-label'>Sessions</div></div>"
        f"<div class='stat'><div class='stat-value'>{primary.get('avg_user_messages', 0)}</div><div class='stat-label'>Msgs/Session</div></div>"
        f"<div class='stat'><div class='stat-value'>{primary.get('avg_duration_min', 0)}</div><div class='stat-label'>Avg Min</div></div>"
        f"<div class='stat'><div class='stat-value'>{primary.get('total_duration_hours', 0)}</div><div class='stat-label'>Span Hours</div></div>"
    )
...
def render_usage_block(narrative: NarrativeBundle) -> str:
    return (
        ...
        f"<div class=\"key-insight\"><strong>Key pattern:</strong> {html.escape(narrative.usage_narrative['key'])}</div>"
        "</div>"
    )
...
def render_chart_rows(data: AnalyzedData, meta: ReportMeta) -> tuple[str, str]:
    ...
    second = (
        ...
        "</div>"
        '<div class="chart-card">'
        f"{block_title_html('Codex Top Domains' if meta.compare_sources else 'Codex Inferred Execution Archetypes', 'chart_execution_archetypes', 'chart-title')}"
...
def render_claude_focus_row(data: AnalyzedData) -> str:
    ...
    return (
        ...
        '<table class="comparison-table"><thead><tr><th>Outcome</th><th>Sessions</th></tr></thead><tbody>'
        f"{outcome_html}"
        "</tbody></table>"
        "</div>"
        "</div>"
...
def render_snapshot_compare(snapshot_compare: dict) -> str:
    ...
    cards = "".join(
        (
            ...
            f"<span>Sessions 变化: {item['sessions_delta']:+}</span>"
            f"<span>平均时长变化: {item['duration_delta']:+} 分钟</span>"
            f"<span>平均消息变化: {item['messages_delta']:+}</span>"
            "</div>"
        )
        for item in items
    ) or '<div class="snapshot-card"><strong>Snapshot Compare</strong><span>暂无上一份快照可比。</span></div>'
    return (
        f"{heading_html('h2', 'Snapshot Compare', 'snapshot', 'section-snapshot')}"
...
def render_trend_cards(trend_cards: list[dict]) -> str:
    ...
    for card in trend_cards:
        ...
        for window_index, window in enumerate(windows):
            ...
            peak_values = " | ".join(
                f'{html.escape(item["name"])} peak {format_metric(max(point["value"] for point in item["points"]))}'
                for item in window["series"]
                if item["points"]
            ) or "暂无数据"
            latest_gap = ""
            ...
            panels.append(
                ...
                f'<div class="trend-window-meta">Window {html.escape(window["label"])} | {html.escape(start_label)} 到 {html.escape(end_label)} | Latest {latest_values} | {peak_values}'
                + (f" | {latest_gap}" if latest_gap else "")
                + " | 左侧纵轴给出最小/中位/峰值参考，图上已标出 peak 和 latest，悬停折线节点可查看每日数值</div>"
                '</div>'
            )
        cards.append(
...
def render_recommendations(cards: list[dict], variant: str = "all") -> str:
    copy = _mode_copy(variant)
    return (
        f"{heading_html('h2', 'Platform Recommendations', 'recommendations', 'section-recommendations')}"
...
def render_quality_score(quality_score: dict) -> str:
    ...
    return (
        f"{heading_html('h2', 'Report Quality Score', 'quality', 'section-quality')}"
...
def render_project_drilldown(cards: list[dict], variant: str = "all") -> str:
    copy = _mode_copy(variant)
    return (
        f"{heading_html('h2', 'Project Drill-Down', 'projects', 'section-projects')}"
        ...
        + "".join(
            (
                ...
                f'<span>{card["sessions"]} sessions | {html.escape(card["split"])}</span>'
...
def render_leaderboards(boards: list[dict]) -> str:
    ...
    return f"{heading_html('h2', 'High-Value Session Leaderboards', 'leaderboards', 'section-leaderboards')}<div class=\"leaderboard-grid\">" + "".join(cards) + "</div>"
    ...
    for group_name, prompts in prompt_library.items():
        items = "".join(
            (
                ...
                f'<div class="copyable-prompt-section"><div class="copyable-prompt-row"><code class="copyable-prompt">{html.escape(item["prompt"])}</code><button class="copy-btn" onclick="copyText(this)">Copy</button></div></div>'
                '</div>'
            )
            for item in prompts
        )
        groups.append(f'<div><p class="section-intro">{html.escape(group_name)}</p><div class="prompt-library-grid">{items}</div></div>')
    return f"{heading_html('h2', 'Prompt Library', 'library', 'section-library')}" + "".join(groups)
...
def render_task_matrix(rows: list[dict]) -> str:
    ...
    return (
        f"{heading_html('h2', 'Task Type Recommendation Matrix', 'matrix', 'section-matrix')}"
        '<table class="comparison-table matrix-table"><thead><tr><th>任务类型</th><th>推荐平台</th><th>观察到的 sessions</th><th>推荐 workflow</th></tr></thead>'
